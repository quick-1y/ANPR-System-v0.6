...
└── anpr/                     # Основной пакет приложения
    ...
    │   ├── __init__.py
    │   ├── anpr_pipeline.py   # Основной пайплайн (включая TrackAggregator с кворумом и сбросом невалидных треков)
...
- **Подавление повторов** — таймер кулдауна с единым кэшем по каналу в основном потоке, исключающий дублирующиеся события даже при параллельном инференсе
