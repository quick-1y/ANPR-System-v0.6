...
- **Экспериментальный OCR-режим** — контурная сегментация символов, пакетный OCR по компонентам и усреднение confidence
...
- **Глобальный режим Debug** — единые переключатели оверлеев (рамки, OCR, треки) и онлайн-панель логов в «Наблюдении», управляется отдельной вкладкой Debug в настройках
...
- **Режим сегментации** — по флагу канала номер разбивается на контурные компоненты/строки с фильтром площади/аспекта, батч CRNN собирается из отдельных символов, а уверенность усредняется по компонентам
...
- В сегментированном режиме между шагами F и G номер разбивается на контурные компоненты (символы/строки), формируется батч CRNN из отдельных кропов и итоговый текст собирается слева-направо с усреднением confidence по всем компонентам.
...
### Параметры сегментации OCR (настраиваются для каждого канала)
- `ocr_segmentation.enabled` — включает экспериментальный режим сегментации символов перед подачей в CRNN.
- `min_symbol_area_ratio` / `max_symbol_area_ratio` — фильтр по относительной площади компонента к кропу номера.
- `min_symbol_aspect_ratio` / `max_symbol_aspect_ratio` — допустимое соотношение сторон контура символа.
- `padding_px` — дополнительный отступ вокруг найденного компонента перед отправкой в батч.
- `max_components` — ограничение числа кропов в батче (символы/строки) для защиты от шумовых контуров.
- `row_merge_threshold` — чувствительность группировки по строкам (от 0.1 до 2.0), влияет на сортировку слева-направо/сверху-вниз.
...
└── anpr/                     # Основной пакет приложения
    ...
    │   ├── __init__.py
    │   └── plate_preprocessor.py # Коррекция наклона/перспективы и сегментация символов перед OCR
    │
