...
flowchart LR
    ...
    C --> M{Проверка мин/макс размера номера}
    M -->|В пределах| D[Сбор бестшотов по треку]
    M -->|Вне диапазона| Z
    D --> K[Выравнивание номера (наклон/перспектива)]
    K --> E[Пакетный OCR CRNN]
    E --> F{Консенсусное решение}
    F -->|Достигнут кворум| P[Валидация и постобработка номера]
    F -->|Недостаточно данных| Z
    P -->|Формат подтверждён| G[Проверка кулдауна]
    P -->|Невалидно| Z
    G -->|Кулдаун прошёл| H[Сохранение в БД и на диск]
    G -->|В кулдауне| Z
    H --> I[Асинхронное обновление UI]
```
...
- **Подавление повторов** — таймер кулдауна для исключения дублирующихся событий
...
[Укажите лицензию вашего проекта, например: MIT License]
